...
import { Await, useLoaderData, useNavigate, useLocation, useParams } from "react-router-dom";
...
function SingleSnippetPage() {
  ...
  // Retrieve the snippetID from the URL using useParams
  const { id: snippetID } = useParams();

  const handleAddComment = () => {
    if (newComment.trim().length < 5) {
      enqueueSnackbar("Comment must be more than 4 characters.", { variant: "error" });
      return;
    }

    if (!snippetID) {
      enqueueSnackbar("Snippet ID is not available. Please try again later.", { variant: "error" });
      return;
    }
              ...
              return (
                <div className="max-w-3xl w-full bg-white rounded shadow p-6">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2 mb-4">
                    {tags.map((tag, index) => (
                      <span key={index} className="bg-blue-600 text-white rounded-md px-2 py-1 text-sm">
                        {tag}
                      </span>
                    ))}
                    <span className="bg-green-600 text-white rounded-md px-2 py-1 text-sm">{genre}</span>
                  </div>
                  ...
                  <div className="mt-6">
                    ...
                    <div className="space-y-3 overflow-y-auto max-h-64 border-t border-gray-200 pt-4" style={{ maxHeight: "16rem" }}>
                      {comments.length > 0 ? (
                    ...
                    {/* Add Comment */}
                    <div className="mt-4">
                      <h4 className="text-lg font-semibold text-gray-700 mb-2">Add a Comment</h4>
                      <textarea
                        value={newComment}
