...
const Navbar = () => {
  ...
  return (
    <nav className='bg-white flex justify-between items-center p-4 h-24 mx-auto text-white'>
      {/* Logo */}
      <h1 className='w-full text-3xl font-bold text-blue-600 cursor-pointer'>SoundSync</h1>

      {/* Desktop Navigation */}
      <ul className='hidden md:flex'>
        {navItems.map(item => (
          <a key={item.id} href={item.href} className='p-4 w-24 hover:bg-blue-600 rounded-xl m-2 cursor-pointer duration-300 text-center text-black hover:text-white'>
            <li>{item.text}</li>
          </a>
        ))}
      </ul>
